...
SECRET_KEY = 'rb*&ed6ppf2n^z)f0h(so2tz*80r_77540yxayz3&q@&=szuzq'
...
CACHES = {
    "default": {
        ...
        "LOCATION": "redis://192.168.146.142:6379/0",
        "OPTIONS": {
    ...
    "session": {
        "BACKEND": "django_redis.cache.RedisCache",
        "LOCATION": "redis://192.168.146.142:6379/1",
        "OPTIONS": {
            "CLIENT_CLASS": "django_redis.client.DefaultClient",
        }
    },
    "verify_codes": {  # 存储短信码
        "BACKEND": "django_redis.cache.RedisCache",
        "LOCATION": "redis://192.168.146.142:6379/2",
        "OPTIONS": {
            "CLIENT_CLASS": "django_redis.client.DefaultClient",
        }
    },
    "history": {  # 存储浏览记录
        "BACKEND": "django_redis.cache.RedisCache",
        "LOCATION": "redis://192.168.146.142:6379/3",
        "OPTIONS": {
            "CLIENT_CLASS": "django_redis.client.DefaultClient",
        }
    },
    "cart": {  # 存储浏览记录
        "BACKEND": "django_redis.cache.RedisCache",
        "LOCATION": "redis://192.168.146.142:6379/4",
        "OPTIONS": {
            "CLIENT_CLASS": "django_redis.client.DefaultClient",
        }
    },
...
# FastDFS
FDFS_BASE_URL = 'http://192.168.146.142:8888/'
...
CRONJOBS = [
    ...
    ('*/1 * * * *', 'contents.crons.generate_static_index_html',
     '>> /Users/gongbinyong/Desktop/item/meiduo/logs/crontab.log')
]
# 解决crontab中文问题
...
HAYSTACK_CONNECTIONS = {
    'default': {
        ...
        'URL': 'http://192.168.146.142:9200/',  # 此处为elasticsearch运行的服务器ip地址，端口号固定为9200
...
ALIPAY_DEBUG = True  # 将来换成真实环境改为False
